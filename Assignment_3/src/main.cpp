...
double dotGridGradient(int ix, int iy, double x, double y)
{
    ...
    double x_val = x_dist * grid[iy][ix][0];
    double y_val = y_dist * grid[iy][ix][1];
    return x_val + y_val;
}
...
double ray_sphere_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    double a_val = ray_direction.dot(ray_direction);
    
    double b_val = ray_direction.dot(ray_origin - sphere_center) * 2.0;

    double rad_square = sphere_radius * sphere_radius;
    double c_val = (ray_origin - sphere_center).squaredNorm()- rad_square;

    double run = b_val * b_val - 4.0 * a_val * c_val;
    ...
    if (run < 0){
        ...
        // 2 intersect pts
        double t1 = (-b_val + sqrt(run)) / (2.0 * a_val);
        double t2 = (-b_val - sqrt(run)) / (2.0 * a_val);

        // pick the pt closest to the ray origin location
        double t = fmin(t1, t2);
...
double ray_parallelogram_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    double t = -1;
    Matrix3d Am_mat;
    Am_mat << pgram_u(0), pgram_v(0), -ray_direction(0),
            pgram_u(1), pgram_v(1), -ray_direction(1),
            pgram_u(2), pgram_v(2), -ray_direction(2);

    Vector3d bm_vector;
    Vector3d ae_vector = ray_origin - pgram_origin;
    bm_vector << ae_vector(0), ae_vector(1), ae_vector(2);

    // Solve for Ax = B
    Vector3d x_vector = Am_mat.colPivHouseholderQr().solve(bm_vector);
    double u_val = x_vector(0), v_val = x_vector(1), tu_val = x_vector(2);

    if (u_val >= 0 && u_val <= 1 && v_val >= 0 && v_val <= 1 && tu_val > 0){
        t = x_vector(2);
        
        p = pgram_origin;
        p += (x_vector(0) * pgram_u);
        p += (x_vector(1) * pgram_v);
...
bool is_light_visible(const Vector3d &ray_origin, const Vector3d &ray_direction, const Vector3d &light_position)
{
    ...
    Vector3d p_vector, n_vector;

    const int nearest_object = find_nearest_object(light_position, -ray_direction, p_vector, n_vector);

    if (nearest_object < 0 || p_vector.isApprox(ray_origin, 2 * ep)){
        return true;
    }
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i){
        ...
        const Vector3d v_vector = (ray_origin - p).normalized();

        const Vector3d h_vector = ((v_vector + Li) / ((v_vector + Li).norm())).normalized();
        
        const Vector4d spec = obj_specular_color * pow(std::max(h_vector.dot(N), 0.0), obj_specular_exponent);

        // Attenuate lights according to the squared distance to the lights
        const Vector3d D = light_position - p;
        lights_color += (diffuse + spec).cwiseProduct(light_color) / D.squaredNorm();
    }
    ...
    if (max_bounce != 0) {
        const Vector3d v_vec = (ray_origin - p).normalized();
        const Vector3d r_vec = ((2 * N * (N.dot(v_vec))) - v_vec).normalized();
        const Vector3d sleep = p + ep * r_vec;
    
        reflection_color = (refl_color.cwiseProduct(shoot_ray(sleep, r_vec, max_bounce -1)));
    }
...
void raytrace_scene()
{
    ...
    //TODO: compute the correct pixels size
